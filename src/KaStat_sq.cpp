...
int main (int argc, char **argv) {
    ...
    /*
    cout << string(argv[3]).substr(0, 1) << endl;
    cout << string(argv[3]).substr((string(argv[3]).length() - 1), 1) << endl;
    cout << string(argv[4]).substr(0, 1) << endl;
    cout << string(argv[4]).substr((string(argv[4]).length() - 1), 1) << endl;
    
    if (string(argv[3]).substr(0, 1) != "\"" || string(argv[3]).substr((string(argv[3]).length() - 1), 1) != "\"" || string(argv[4]).substr(0, 1) != "\"" || string(argv[4]).substr((string(argv[4]).length() - 1), 1) != "\"") {
        cout << "For the rules' names, please put inside double quotation marks." << endl;
        exit(EXIT_FAILURE);
    }
    */
